...
def test_sankey_data_money_flow(client):
    """Ensure Sankey data aggregates transactions into correct flows."""
    # Create a fund which also sets up a corresponding category
    assert client.post('/api/funds', json={'name': 'Travel Fund'}).status_code == 200

    # Map category names to ids for convenience
    resp = client.get('/api/categories')
    cat_map = {c['name']: c['id'] for c in resp.get_json()}

    transactions = [
        {'amount': '100', 'transaction_type': 'income', 'category_id': cat_map['Gross Salary'], 'date': '2024-01-01'},
        {'amount': '20', 'transaction_type': 'deduction', 'category_id': cat_map['401k Deduction'], 'date': '2024-01-02'},
        {'amount': '30', 'transaction_type': 'expense', 'category_id': cat_map['Groceries'], 'date': '2024-01-03'},
        {'amount': '10', 'transaction_type': 'expense', 'category_id': cat_map['Gas'], 'date': '2024-01-04'},
        {'amount': '15', 'transaction_type': 'fund_contribution', 'category_id': cat_map['Travel Fund'], 'date': '2024-01-05'},
    ]

    for tx in transactions:
        assert client.post('/api/transactions', json=tx).status_code == 200

    resp = client.get('/api/sankey-data/monthly/2024-01')
    assert resp.status_code == 200
    data = resp.get_json()

    node_idx = {n['name']: i for i, n in enumerate(data['nodes'])}

    def link_value(src, tgt):
        s, t = node_idx[src], node_idx[tgt]
        for l in data['links']:
            if l['source'] == s and l['target'] == t:
                return l['value']
        return None
    ...
    assert link_value('Income', 'Savings') == 15
    assert link_value('Food', 'Groceries') == 30
    assert link_value('Transportation', 'Gas') == 10
    assert link_value('Savings', 'Travel Fund') == 15
